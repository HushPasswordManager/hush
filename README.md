...
- [ ] Standards-based. Hush is based on widely-used FOSS standards such as msgpack, all of its encryption choices (see [Security](#Security)), and its persistent storage format (SQLite). 
  - [ ] Extensible.
  - [ ] Safe.
- [ ] Support both hardware (e.g. [Yubikey](https://www.yubico.com/)) and software (e.g. [Authy](https://authy.com/)) 2-factor authentication
- [ ] Optional web interface. It should be easy to deploy Hush to a web server that presents a secure, performant user interface.
- [ ] Support migration from existing applications. Hush uses basic nested key-value stores with associated tags, so it should be able to integrate all the data from any of the following applications. Support is underway for easy migration.
  - [ ] 1Password
  - [ ] BitWarden
  - [ ] Dashlane
  - [ ] KeyPass
